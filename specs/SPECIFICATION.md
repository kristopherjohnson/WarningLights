...
|--------|-------------------|---------|
| Memory | System memory pressure reaches `.critical` level | Event-driven via `DispatchSource.makeMemoryPressureSource`, supplemented by 60-second timer for UI refresh |
...
- **Icon color**:
  ...
  - Warning active: `.palette` rendering with orange on all symbol layers (`.foregroundStyle(.orange, .orange, .orange)`); this makes the entire icon opaque orange, immediately distinguishable at a glance
...
- **Memory (pressure detection)**: Use `DispatchSource.makeMemoryPressureSource(eventMask: .all, queue: .main)` to receive kernel memory pressure events. This is the only public, supported API that reports the kernel's actual memory pressure determination. Three levels exist: `.normal`, `.warning`, `.critical` (Swift `DispatchSource.MemoryPressureEvent`). Trigger the warning icon only at `.critical` — this is when the kernel's jetsam thread begins evaluating processes for termination and the system is genuinely constrained. The `.warning` level fires frequently during normal use (cache eviction, compressor activity) and is not user-actionable. Do NOT use percentage-based heuristics to determine alert state.
...
The application bundle icon (shown in Finder, Launchpad, and the About panel) uses:

- **Background**: Dark brownish (`#3B2A1A`)
- **Foreground**: Solid orange (`#FF8C00`) triangle with rounded corners; no exclamation mark or other decoration

This icon is distinct from the menu bar extra icon (which uses SF Symbols at runtime). The app icon is a static `.icns` asset included in the bundle. The icon PNGs are generated by `specs/generate_icon.swift` (run with `swift specs/generate_icon.swift`).
